...
# drop categoricals
nums <- sapply(imdb5k.clean, is.numeric)
imdb5k.clean <- imdb5k.clean[, nums]
```


```{r}
imdb5k.mldr <- mldr_from_dataframe(dataframe=imdb5k.clean, labelIndices=which(startsWith(colnames(imdb5k.clean), "i_")))
```
...
# Predict
val <- predict(rakel.model, ds$val, cores=parallel::detectCores())
test <- predict(rakel.model, ds$test, cores=parallel::detectCores())
